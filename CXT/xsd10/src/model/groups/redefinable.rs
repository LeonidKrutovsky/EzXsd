...
impl Redefinable {
    pub const NAMES: &'static [&'static str] = &[
        TopLevelSimpleType::NAME,
        TopLevelComplexType::NAME,
        Group::NAME,
        AttributeGroup::NAME,
    ];
    ...
    pub fn parse(node: roxmltree::Node<'_, '_>) -> Result<Self, String> {
        match node.tag_name().name() {
            TopLevelSimpleType::NAME => {
                Ok(Self::SimpleType(Box::new(TopLevelSimpleType::parse(node)?)))
            }
            TopLevelComplexType::NAME => Ok(Self::ComplexType(Box::new(
                TopLevelComplexType::parse(node)?,
            ))),
            Group::NAME => Ok(Self::Group(Box::new(Group::parse(node)?))),
            AttributeGroup::NAME => {
                Ok(Self::AttributeGroup(Box::new(AttributeGroup::parse(node)?)))
            }
